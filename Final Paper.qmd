...
To better understand the contribution of different feature groups, separate base models were initially created for apartment-level and neighborhood-level datasets. The Apartment Base Model included features such as bathrooms, bedrooms, interest level, outdoor space, dishwasher availability, elevator access, pool access, and furnished status. The Neighborhood Base Model included variables such as median income, vacancy rate, green score, movement score and restaurant score. After evaluating the results from these independent models, the apartment and neighborhood datasets were merged into a combined dataset. Ridge regression, LASSO regression, and Random Forest models were then applied again to evaluate performance on the updated dataset and identify the strongest overall predictors of rental price.
...
We found that the the Apartment Base Model performed poorly with their respective R² scores. The Ridge regression had an R² of 0.4535 and the LASSO regression had an R² of 0.4555. The random forest performed better, as expected, obtaining an R² score of 0.6085 (see Figure 1).

![Figure 1: Comparison of R² values for base model including all variables.](images/clipboard-1348377745.png)

Afterwards, removing insignificant features calculated by random forest we manage to raise the model's R² score of random forest to .6233 (see Figure 2).

![Figure 2: The best features of the final random forest model, based solely on unit-level features](images/clipboard-3734808285.png)
...
To try and isolate the effects of the underlying neighborhood features, we introduced composite scores and re-ran our model. Composite scores were created to reduce the number of related variables and make the model easier to interpret. Before creating these scores, all numeric variables were standardized using z-scores, so each variable was measured in standard deviation units rather than its original scale. Each composite score was then calculated as the average of the standardized variables within that category. For example, the movement score was calculated as the average of standardized walk score, transit score, bike score, subway station count, and average routes per station. The same process was used for restaurant and green space variables. An education score was also created, but it was excluded from the final alternate model because too many neighborhoods were missing education data. We also decided to exclude sale price per unit in the revised regression to see if we could maintain explanatory power with the less impactful predictors. We were pleased to see this model maintain strong explanatory power (R² ≈ 0.83) and identified median income, vacancy rate, and green space as significant predictors of rent, while movement and restaurant variables were not statistically significant. These results suggest that, beyond property values, economic conditions and broader geographic features play the most important role in determining rental prices.

Once the neighborhoods dataset was thoroughly analyzed, we extracted the key features the LASSO regression had isolated; median_income, green_score, and vacancy_rate which we joined onto the apartment dataset. As before, we began doing variable selection by using Random Forest regressions to capture the significant features within the combined model. We found that from the new predictors median_income, and vacancy_rate where significant predictors within the combined model, while, green_score was calculated as insignificant by the random forest, and so was excluded as a predictor going forward (see Figure 3).

![Figure 3: Our final random forest model on the combined dataset, with only significant predictors. R² = 0.748](images/clipboard-4227080777.png)

Finally, we then used these variables to create a linear regression model, regressing on log(price) because we suspected the dataset to be right skew. The model performed decently, obtaining an adjusted R² of 0.7046. However, it had clear issues with heteroskedasticity, and despite logging price, wide tails and right skew (see Figure 4 and 5).

![Figure 4: Plot showing clear heteroskedasticity in the linear regression model](images/clipboard-57054439.png)

![Figure 5: Plot showing clear wide-tails and right skew in the linear regression model](images/clipboard-3651035174.png)

We also calculated the VIF of each variable, which came out to be under the threshold for problematic multicollinearity for each predictor, and created component residual plots showing that bathrooms had a non-linear effect. Because the heteroskedasticity was so extreme, we suspected there was a confounding variable we had missed. To counteract this, we added in the variable dist_to_cbd_km, which measured the distance from any given unit to the World Trade Center. We hoped this variable would capture some of the value of neighborhoods in NYC that was causing the heteroskedasticity, wide tails, and right skew. Additionally, we added interaction effects into the model for bathrooms, bedrooms, elevator, in unit washer and dryer, fitness center, outdoor space and doorman. The variable dist_to_cbd_km is coded so that it increases the further away from the world trade center the unit is. We then ran an F-test over these interaction effects. Which was highly significant (see Figure 6).

![Figure 6: F-test over the interaction effects](images/clipboard-4071534833.png)
